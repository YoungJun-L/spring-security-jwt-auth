...
class VerificationCodeServiceTest(
    ...
    private val accountRepository: AccountRepository,
) : FunSpec(
        {
            ...
            context("새 계정 인증 코드 생성") {
                test("성공") {
                    val actual = verificationCodeService.generateForNewAccount(EMAIL_ADDRESS)
                ...
                test("가입된 이메일 주소가 존재하면 실패한다.") {
                    ...
                    shouldThrow<AuthException> { verificationCodeService.generateForNewAccount(EMAIL_ADDRESS) }
                ...
                test("10분 내 발급된 인증 코드가 5개 이상이면 실패한다.") {
                    ...
                    shouldThrow<AuthException> {
                        verificationCodeService.generateForNewAccount(
                            EMAIL_ADDRESS,
                            verificationCodes.first().createdAt + 10.minutes - 1.seconds,
                        )
                    }.errorType shouldBe VERIFICATION_CODE_LIMIT_EXCEEDED
                }
            }

            context("기존 계정 인증 코드 생성") {
                test("성공") {
                    accountRepository.save(AccountBuilder(emailAddress = EMAIL_ADDRESS).build())

                    val actual = verificationCodeService.generateForExistingAccount(EMAIL_ADDRESS)
                ...
                test("계정이 이용 제한된 상태면 실패한다.") {
                    accountRepository.save(AccountBuilder(emailAddress = EMAIL_ADDRESS, status = AccountStatus.DISABLED).build())

                    shouldThrow<AuthException> { verificationCodeService.generateForExistingAccount(EMAIL_ADDRESS) }
                ...
                test("10분 내 발급된 인증 코드가 5개 이상이면 실패한다.") {
                    ...
                    shouldThrow<AuthException> {
                        verificationCodeService.generateForExistingAccount(
                            EMAIL_ADDRESS,
                            verificationCodes.first().createdAt + 10.minutes - 1.seconds,
                        )
                    }.errorType shouldBe VERIFICATION_CODE_LIMIT_EXCEEDED
